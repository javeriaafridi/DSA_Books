...
#include <sstream>
using namespace std;
void loadBooksFromFile(Node*& root);
void saveBooksToFile(Node* root);
void loadBooksFromFile(Node*& root) {
    ifstream file("books.txt");

    if (!file.is_open()) {
        cout << "No book file found. Starting empty.\n";
        return;
    }
    ...
    while (getline(file, line)) {
        ...
        if (searchByBookID(root, id) != nullptr) {
            continue;
        }

        // Use BST insert (DO NOT MODIFY BST)
        root = insertBST(root, title, author, id);
    }
...
void saveInOrder(Node* root, ofstream& file) {
    ...
    file << root->title << "," << root->author << "," << root->id << endl;
